...
export async function getOrganizerApplicationSetupEmailData(applicationId: string) {
  const application = await prisma.organizerApplication.findUnique({
    where: { id: applicationId },
    include: {
      approvedUser: {
  ...
  if (!application || application.removedAt) {
    throw new Error("申请不存在，或已经被移除。");
  }

  if (application.status !== "APPROVED" || !application.approvedUser) {
    throw new Error("该申请尚未批准，不能发送设密邮件。");
  }

  if (application.approvedUser.status !== "ACTIVE") {
    throw new Error("该主办方账号当前不可用，不能发送设密邮件。");
  }

  if (application.approvedUser.passwordHash) {
    throw new Error("该账号已经完成首次设密，无需再次发送设密邮件。");
  }

  const setupToken = application.approvedUser.passwordSetupTokens[0];

  if (!setupToken) {
    throw new Error("当前没有可用的设密链接，可能已经过期。请删除权限后让申请人重新申请。");
  }

  return {
    to: application.approvedUser.email,
    contactName: application.approvedUser.name || application.contactName,
    organizationName: application.organization?.name || application.organizationName,
